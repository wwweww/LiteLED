...
esp_err_t led_strip_init( led_strip_t* strip ) {
    ...
    if ( !strip->buf ) {
        log_e( "Cannot init strip - insufficient memory." );
...
esp_err_t led_strip_set_pixels_c( led_strip_t* strip, size_t start, size_t len, crgb_t* data ) {
    CHECK_ARG( strip && strip->buf && len && start + len <= strip->length );
    for ( size_t i = 0; i < len; i++ )
        CHECK( led_strip_set_pixel( strip, i + start, rgb_from_code( data[i] ) ) );
...
uint8_t led_strip_get_brightness( led_strip_t* strip ) {
    if ( !( strip && strip->buf ) ) {
        log_e( "getBrightness(): strip not initialized." );
        return 0;
    }
    uint8_t res = strip->bright_act;
    return res;
}
